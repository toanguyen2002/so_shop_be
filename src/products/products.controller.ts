...
export class ProductsController {
  ...
  @Public()
  @Get('/seller/:id/:page')
  async getAllroductBysellerId(
    @Param('id') seller: string,
    @Param('page') page: any,
  ): Promise<any> {
    // console.log(rs);
    const start = 20 * (page - 1);
    const end = start + 20;
    return (await this.productService.getProductBySellerId(seller)).slice(
  ...
  async updateProducts(
    @Body() ProductsDTO: ProductsUpdateDTO,
  ): Promise<Products> {
