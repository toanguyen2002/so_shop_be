...
export class WalletService {
  ...
  async increBalance(walletDTO: WallerDTO): Promise<Wallet> {
    ...
    if (rs.length > 0) {
      ...
      const newU = await this.createBalance({
        balance: 0,
        user: walletDTO.user,
      });
      const newrs = await this.model.aggregate([
        { $match: { user: new mongoose.Types.ObjectId(newU.user.toString()) } },
      ]);
      newrs[0].balance += walletDTO.balance;
      console.log(newrs, 'newUnewUnewU');

      await this.model.findByIdAndUpdate(newrs[0]._id, newrs[0]);
    }
    return rs[0];
  }
